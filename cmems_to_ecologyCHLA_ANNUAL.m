% M. De Dominicis (2020)
% monthly averages from CMEMS data of Chl-a for ORE Supergen

clear all, close all
path(path,'/login/micdom/matlab/m_map')
basedir=('/scratch/micdom/CMEMS_data_releaseDec2020/Chla/')

root_name=('metoffice_foam1_amm7_NWS_CPWC_dm')
...
dummyfile='/scratch/micdom/CMEMS_data_releaseDec2020/Chla/1998/01/metoffice_foam1_amm7_NWS_CPWC_dm19980101.nc'
  ...
  for Month=1:12
      ...
      for Day=1:dmonth
          if Day<10;dd=['0' num2str(Day) ];else;dd=num2str(Day);end
           ...
           chl=ncread(fname,'chl');
           chla_max(:,:,Day)=max(chl,[],3);     
           VAR=chla_max;
...
for year=1:YY
y1=Year(year,:);
Chla_mean(:,:,:,year)=ncread(['/scratch/micdom/CMEMS_data_releaseDec2020/postproc/Chla_' y1 '.nc'],'Chla_mean');
Chla_max(:,:,:,year)=ncread(['/scratch/micdom/CMEMS_data_releaseDec2020/postproc/Chla_' y1 '.nc'],'Chla_max');
end
...
%pcolor(squeeze(Chla_mean(:,:,Month,year))'); shading flat; caxis([0 15]);colormap(jet);%cmocean('thermal',20);
pcolor(squeeze(Chla_max(:,:,Month,year))'); shading flat; caxis([0 15]);colormap(jet);%cmocean('thermal',20);
