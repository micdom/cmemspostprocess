% M. De Dominicis (2020)
% monthly averages from CMEMS data of netPP for ORE Supergen

clear all, close all
path(path,'/login/micdom/matlab/m_map')
basedir=('/scratch/micdom/CMEMS_data_releaseDec2020/netPP/')

root_name=('metoffice_foam1_amm7_NWS_PPRD_dm')
...
dummyfile='/scratch/micdom/CMEMS_data_releaseDec2020/netPP/1998/01/metoffice_foam1_amm7_NWS_PPRD_dm19980101.nc'
...
%thick_mtx_corr=nan(297,375,24);
for i=1:297
 for j=1:375
 last=find(isnan(netPP(i,j,:)),1,'first');                     
 if length(last)>=1 & last<24
 bathymod(i,j)=depth(last);
 %thick_mtx_corr(i,j,1:last-1)=thick_mtx(i,j,1:last-1);
 %thick_mtx_corr(i,j,last)=thick_mtx(i,j,last)-(bathymod(i,j)-bathy(i,j));
 else
 %thick_mtx_corr(i,j,1:last-1)=thick_mtx(i,j,1:last-1);  
  ...
  for Month=1:12
      ...
      for Day=1:dmonth
          if Day<10;dd=['0' num2str(Day) ];else;dd=num2str(Day);end
           ...
           netPP=ncread(fname,'nppv');

           
             for lev=1:23
             thickness=depth(lev+1)-depth(lev)
             netPPa(:,:,lev)=netPP(:,:,lev)*thickness;
             %depth averaging to double-check
             end
             netPPa_mean(:,:,Day)=(nansum(netPPa,3)./bathymod).*(bathymod*365/1000);
             %netPPa_mean(:,:,Day)=(nansum(netPPa,3)./bathymod).*(365/1000);

             %netPPa_mean(:,:,Day)=(nansum(netPPa,3)./bathymod);
             
             %netPPa_mean2(:,:,Day)=(nansum(netPPa,3)./bathy);
             %netPPa3=netPP.*thick_mtx_corr;
             %netPPa_mean3(:,:,Day)=(nansum(netPPa3,3)./bathy);
             
%              %pcolor(squeeze(netPPa_mean(:,:,Day))-nansum(netPP,3));shading flat
%              %figure;pcolor(netPPa_mean(:,:,Day)'-netPPa_mean3(:,:,Day)');shading flat;caxis([-10 10])
%             
%              %figure;pcolor(bathymod-bathy);shading flat
...
for year=1:YY
y1=Year(year,:);
netPP_mean(:,:,:,year)=ncread(['/scratch/micdom/CMEMS_data_releaseDec2020/postproc/netPP_' y1 '.nc'],'netPP_mean');
%netPP_max(:,:,:,year)=ncread(['/scratch/micdom/CMEMS_data_releaseDec2020/postproc/netPP_' y1 '.nc'],'netPP_max');
end
...
pcolor(squeeze(netPP_mean(:,:,Month,year))'); shading flat; caxis([0 500]);colormap(jet);%cmocean('thermal',20);
%pcolor(squeeze(netPP_max(:,:,Month,year))'); shading flat; caxis([0 500]);colormap(jet);%cmocean('thermal',20);
